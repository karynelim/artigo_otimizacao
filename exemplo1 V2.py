...
p = [3,2,3,1,2,3,1,2,3,1,2,3,1,3,2]
t = [8,5,10,4,6,7,2,3,5,4,9,8,5,6,7]

prof = [
    [3,2,1,2,3,2,1,3,2,1,3,3,1,2,3],
    [2,3,2,3,2,1,2,1,3,2,1,2,3,1,2],
    [1,1,3,2,1,3,3,2,1,3,2,1,2,3,1]
]
...
# Perturbação: muda uma tarefa para outro membro aleatoriamente
...
# ILS principal com tempo até primeira ótima
def ILS(max_iter=1000):
    ...
    for iteracao in range(1, max_iter+1):
        ...
        if novo_beneficio > melhor_beneficio:
            ...
            melhor_beneficio = novo_beneficio
            tempo_primeira_vez = time.time() - tempo_inicio

    if tempo_primeira_vez == 0.0:
        tempo_primeira_vez = time.time() - tempo_inicio

    return melhor_solucao, melhor_beneficio, tempo_primeira_vez

# Execução
start_time = time.time()
solucao_final, beneficio_final, tempo_ate_otima = ILS(max_iter=1000)
end_time = time.time()
...
print("Tempo total de execução (segundos):", round(end_time - start_time, 4))
print("Tempo até encontrar a solução ótima (segundos):", round(tempo_ate_otima, 4))
